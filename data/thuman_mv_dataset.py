...
class Thuman_mvDataset(BaseDataset):
    ...
    def initialize(self, opt, phase):
        ...
        self.phase = phase
        self.data_root = os.path.join(opt.dataset.data_root, f'thuman2.1_render_512')
        ...
        self.source_list = opt.dataset.source_list
    ...
    def get_list(self,):
        if self.phase == 'train':
            with open(os.path.join(self.opt.dataset.data_root, f"thuman2.1_{self.phase}.txt"), 'r') as f:
                data = [i.rstrip("\n").split("_2_")[0] for i in f.readlines()]
            data = list(set(data))
        elif self.phase == 'test':
            with open(os.path.join(self.opt.dataset.data_root, f"thuman2.1_val_test.txt"), 'r') as f:
    ...
    def train_data(self, index):
        ...
        dcit_tensor['source_view'] = self.load_multi_view(img_path,
                                                          ...
                                                          require_depth_smpl=True)
        target = np.random.randint(low=0, high=36)
        dcit_tensor['target_view_novel'] = self.get_single_novel_view_tensor(img_path, f"view{target}.png")
        dcit_tensor["pair"] = f"{id}_2_view{target}"
        # pc,pc_mask = get_pointcloud(img_path)
        # dcit_tensor['source_view']["pc"] = pc.astype(np.float32)
        # dcit_tensor['source_view']["pc_mask"] = pc_mask
    ...
    def val_data(self, index):
        ...
        dcit_tensor['source_view'] = self.load_multi_view(img_path,
                                                          ...
                                                          require_spmlx=False, phase=self.phase,
                                                          require_depth_smpl=True)
        dcit_tensor['target_view_novel'] = self.get_single_novel_view_tensor(img_path, target)
        dcit_tensor["pair"] = f"{id}_2_{target[:-4]}"
    ...
    def load_multi_view(self, img_path, source_list, resize=None, require_mask=True,
                        require_depth=True, require_spmlx=True, phase="test",require_depth_smpl=False):
        source_dict = {}

        for source in source_list:
            out = self.load_single_view(img_path, source, resize=resize, require_mask=require_mask,
                                        require_depth=require_depth,require_depth_smpl=require_depth_smpl,
                                        require_spmlx=require_spmlx, phase=phase)
    ...
    def load_single_view(self, img_path, sample_name, resize=None, require_mask=True,
        ...
        pose_img, mask, depth, pc, depth_smpl = None, None, None, None, None

        # pose_img_name = img_name[:-4]+"_smpl.png"
        pose_img_name = img_name[:-4]+"_normal.png"
        pose_img = read_img(pose_img_name)
        pose_img = torch.from_numpy(pose_img).permute(2, 0, 1) / 255.0 * 2.0 - 1.0
        if require_depth:
            depth = cv2.imread(depth_name, cv2.IMREAD_UNCHANGED).astype(np.float32) / 2.0 ** 15
            depth = torch.from_numpy(depth)
        if require_depth_smpl:
